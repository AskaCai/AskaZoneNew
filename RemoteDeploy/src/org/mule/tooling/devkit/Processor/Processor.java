...
public class Processor {
	...
	public void getProcessorProperty(){
		//创建**-processor.properties文件
		File output = new File(projectPath+File.separator+"processor.properties");
		log.appendLog("准备创建processor.properties文件：" + projectPath+File.separator+"processor.properties");
	    ...
	    log.appendLog("processor.properties文件创建成功：" + projectPath+File.separator+"processor.properties");
	    
	  //通过editor.xml获取processor信息
	    PrintWriter pw = null;
	    try {
			...
			log.appendLog("准备往" + output.getName() + "写入processor信息");
			//获取组件名
			command = "/namespace";
			...
			//获取插件内@processor方法及其参数 简单期间，假设一个插件只有一个@processor方法
			...
			pw.close();
			log.appendLog("往" + output.getName() + "写入processor信息完成");
		...
		log.appendLog("准备将"+zipName+"-processor.properties压缩到"+zipName+".zip中");
		ZipUtils.addFileToExistZip(zipFile, output, zipName+"-processor.properties");
		log.appendLog(zipName+"-processor.properties压缩完成");
